...
| Worker logs `outbound` but nothing arrives | the number may be restricted by WhatsApp | Send a message by hand from the bot's own phone. If that does not arrive either, the account is the problem, not the code |
